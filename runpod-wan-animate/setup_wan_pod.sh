...
# - Network Volume attached at /workspace with the HuggingFace official
#   Wan2.2-Animate-14B model already downloaded under
#   /workspace/Wan2.2-Animate-14B/ (see main README step 6). The layout
#   we expect (sharded HF format, not ComfyUI single-file repacks):
#     /workspace/Wan2.2-Animate-14B/diffusion_pytorch_model-0000{1..4}-of-00004.safetensors
#                                                                                     (~32 GB total)
#     /workspace/Wan2.2-Animate-14B/models_t5_umt5-xxl-enc-bf16.pth                   (~11 GB)
#     /workspace/Wan2.2-Animate-14B/models_clip_open-clip-xlm-roberta-large-vit-huge-14.pth
#                                                                                     (~4.5 GB)
#     /workspace/Wan2.2-Animate-14B/Wan2.1_VAE.pth                                    (~485 MB)
#     /workspace/Wan2.2-Animate-14B/process_checkpoint/det/yolov10m.onnx
#     /workspace/Wan2.2-Animate-14B/process_checkpoint/pose2d/vitpose_h_wholebody.onnx
...
# HuggingFace official Wan2.2-Animate-14B layout (sharded safetensors +
# .pth files, NOT ComfyUI single-file repacks). The current server.py
# constructs WanAnimate directly from this checkpoint dir via the
# Wan-Video repo's WAN_CONFIGS["animate-14B"] config.
#
...
if [ "$missing" -gt 0 ]; then
    err ""
    err "$missing required weight file(s) missing under ${WAN_CKPT_DIR_OVERRIDE}/"
    err "If your model lives elsewhere on the volume:"
    err "    export WAN_CKPT_DIR_OVERRIDE=/workspace/your-path && bash setup_wan_pod.sh"
    err "If the model is missing entirely, follow the bootstrap in main README step 6."
    exit 1
fi
log ""
...
# Point server.py at the HuggingFace official model layout on the
# Network Volume. server.py reads WAN_CKPT_DIR and constructs the Wan
# 2.2 Animate pipeline directly from it (sharded safetensors + .pth
# files in HF format; process_checkpoint/ for det/pose2d/sam2 ONNX).
#
# WAN_CKPT_DIR_OVERRIDE at install time (above) lets you point this at
# a non-default location; we bake the resolved path into start.sh so
# subsequent restarts don't need the env var.
export WAN_REPO=/opt/wan22
export WAN_CKPT_DIR=${WAN_CKPT_DIR_OVERRIDE}
export SAM2_CKPT_DIR=${WAN_CKPT_DIR_OVERRIDE}/process_checkpoint/sam2
